...
# Determine if a reboot will be needed early, based on the gathered state.
$rebootWillBeRequired = ($null -ne $script:systemState.S1AgentInfo.Service)
...
if ($rebootWillBeRequired) {
    Write-Host "[$ScriptName] Active S1 Windows Services exist. A post-uninstall reboot is mandatory..."
    
    # Pass the entire state object to our decision function.
    Write-Host "[$ScriptName] [DECISION] Evaluating pre-action safety using comprehensive decision logic..."
    $preActionDecision = Test-C9RebootDecision -Scenario PreAction -SystemState $script:systemState -OverrideSuppression $true
...
try {
    ...
    # pre-flight reboot clearance - Use the pre-gathered state
    ...
    if ($clearPendingDecision.ShouldReboot) {
        ...
        try {
            ...
            # Our $script:systemState variable WILL survive this reboot.
    ...
    if ($rebootWillBeRequired) {
        ...
        # We need to refresh user activity right before the final reboot decision.
        Write-Host "[$ScriptName] Refreshing user activity state before final reboot decision..."
