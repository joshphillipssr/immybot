...
Write-Host "[$ScriptName] Starting SentinelOne Uninstallation..."
try {
    ...
    try { Test-MsiExecMutex -ErrorAction Stop
    ...
    Write-Host "[$ScriptName] Uninstallation completed successfully."
    return $true

} catch {
    $errorMessage = "[$ScriptName] The Uninstallation failed with a fatal error: $($_.Exception.Message)"
    throw "[$ScriptName] $errorMessage"
