...
$filename = 'get-user-defined-event-log' # Default filename for output (without extension)
...
$scriptfunction = {
    ...
    function Convert-EventLogRecordToCsvFriendly {
        param (
            [Parameter(Mandatory=$true)]$record
        )
        $result = [PSCustomObject]@{
            Message = $record.Message -replace "`r`n", "\r\n" -replace "`t", "\t" -replace ",", "" -replace "`n", "\n"
            Id = $record.Id
            ...
            TimeCreated = (Get-Date $record.TimeCreated)
            ActivityId = $record.ActivityId
            RelatedActivityId = $record.RelatedActivityId
            ContainerLog = $record.ContainerLog
            MatchedQueryIds = ($record.MatchedQueryIds -join ",")
            LevelDisplayName = $record.LevelDisplayName
            OpcodeDisplayName = $record.OpcodeDisplayName
            TaskDisplayName = $record.TaskDisplayName
            KeywordsDisplayNames = ($record.KeywordsDisplayNames -join ",")
        }

        # Filter out empty or null values
        $filteredResult = @{}
        $result.PSObject.Properties | ForEach-Object {
            if ($null -ne $_.Value -and $_.Value -ne "" -and $_.Value -ne @()) {
                $filteredResult[$_.Name] = $_.Value
            }
        }

        return [PSCustomObject]$filteredResult
    }

    $output = Get-WinEvent -LogName $logName | ForEach-Object {
        Convert-EventLogRecordToCsvFriendly $_
    }
