...
$filename = 'installed-apps' # Default filename for output (without extension)
...
$scriptfunction = {
    ...
    # Collect applications from registry (32 + 64 bit applications)
    # Define suffixes
    $64BitAppsSuffix = "\Software\Microsoft\Windows\CurrentVersion\Uninstall\"
    $32BitAppsSuffix = "\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\"

    # Initialize arrays to store the resulting paths
    $registryAppPaths = @()

    # Concatenate each subkey with the suffixes and store in "registryAppPaths"
    # Iterate over HKEY_USERS sids and HKLM
    foreach ($userSubkey in ((Get-ChildItem -Path "Registry::HKEY_USERS") + "HKLM")) {
        $registryAppPaths += $userSubkey.ToString() + $64BitAppsSuffix
        $registryAppPaths += $userSubkey.ToString() + $32BitAppsSuffix
    }

    $registryApps = @()
    foreach ($basePath in $registryAppPaths) {
        if (Test-Path -Path Registry::$basePath) {
            $withWildCard = ("$basePath" + "*")
            $registryApps += Get-ItemProperty -Path Registry::$withWildCard | Select-Object DisplayName, DisplayVersion, Publisher, InstallDate, InstallSource
        }
    }

    # Collect applications using WMI (Win32_Product)
    $wmiApps = Get-WmiObject -Class Win32_Product | Select-Object -Property Name, Vendor, Version, InstallDate, InstallSource

    # Collect applications installed via package managers (e.g., MSIX, AppX)
    $packageApps = Get-Package | Select-Object -Property Name, Version
    ...
    function Get-ObjectHash {
        ...
        if ($obj.Name -ne $null) {
            ...
            return ($obj.DisplayName + $obj.DisplayVersion).GetHashCode()
        }
    }
    ...
    foreach ($app in $allApps) {
        # Compute the hash code for the current application object
        $hashCode = Get-ObjectHash $app

        # Check if the hash code is not already in the hashtable
        if (-not $hashSet.ContainsKey($hashCode)) {
            # Add the hash code to the hashtable
            $hashSet[$hashCode] = $true

            # Add the application to the unique array
            $uniqueObjects += $app
        }
    }

    # Ensure you write-output of the object so it is passed back to common for formatting and output
    Write-Output $uniqueObjects
